      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-xl shadow-slate-200/50 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          <div className="flex flex-wrap items-center gap-4 mb-8 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
            ...
            <Button type="primary" className="bg-blue-600 hover:bg-blue-500 h-10 px-6 rounded-lg shadow-blue-200">
              查询
            </Button>
          </div>
            ...
            columns={[
              ...
              { 
                ...
                render: (v: RecordItem['status']) => {
                  const colorMap = {
                    '已出题': { color: 'success', bg: 'bg-green-50', text: 'text-green-600', border: 'border-green-100' },
                    '进行中': { color: 'processing', bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-100' },
                    '失败': { color: 'error', bg: 'bg-red-50', text: 'text-red-600', border: 'border-red-100' },
                  };
                  const style = colorMap[v] || colorMap['进行中'];
                  return (
                    <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${style.bg} ${style.text} border ${style.border}`}>
                      <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${v === '已出题' ? 'bg-green-500' : v === '进行中' ? 'bg-blue-500' : 'bg-red-500'}`}></span>
                      {v}
                    </div>
                  );
                } 
              },
              { title: '押题类型', dataIndex: 'type', render: text => <span className="text-slate-600">{text}</span> },
              { 
                title: '难度等级', 
                dataIndex: 'level',
                render: text => (
                  <span className={`font-medium ${text === '进阶' ? 'text-purple-600' : text === '中级' ? 'text-blue-600' : 'text-slate-600'}`}>
                    {text}
                  </span>
                )
              },
              { title: '公司名称', dataIndex: 'company', render: text => <span className="font-bold text-slate-800">{text}</span> },
              { title: '岗位名称', dataIndex: 'job', render: text => <span className="text-slate-600">{text}</span> },
              { title: '押题时间', dataIndex: 'time', render: text => <span className="text-slate-500 text-sm font-mono">{text}</span> },
              { 
                title: '操作', 
                render: (_: any, row: any) => (
                  <Space size="small">
                    <a href={`/user/press/${row.key}`} className="text-blue-600 hover:text-blue-500 font-medium">查看详情</a>
                    <span className="text-slate-300">|</span>
                    <Button type="link" className="text-slate-500 hover:text-blue-600 p-0 h-auto font-normal">继续押题</Button>
                  </Space>
                ) 
              },
            ]}
          ...
          {selectedKeys.length > 0 && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-xl flex items-center gap-2 text-blue-700 text-sm animate-fade-in-up">
              ...
              <Button size="small" type="text" className="text-blue-600 hover:text-blue-800 hover:bg-blue-100/50">批量导出</Button>
              <Button size="small" type="text" danger className="hover:bg-red-50">批量删除</Button>
            </div>
          )}
        </div>
      </div>
