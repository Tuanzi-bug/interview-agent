...
1. 自我介绍
1. 项目做完了吗？背景是什么？项目初期的背景调研是怎么做的？现在这个系统做到哪一步了？
1. 用户下单用户派送的优劣了解过吗？怎么管理？
1. 项目里面遇到的最大的难题是什么？为什么？
1. 超卖，重复下单怎么解决的？
**超卖问题**：
...
1. 最开始有没有考虑乐观锁的适用场景和悲观锁的适用场景？
1. 乐观锁会不会导致频繁的冲突啊？这种情况下和悲观锁谁的性能更好一些呢？
...
1. 一开始为什么没考虑到呢？是调研不充分吗？
1. Redis怎么解决超卖问题的？为什么能解决这个问题？
主要通过以下几种方式：
...
1. 有个场景你了解吗：锁获取后程序退出了，这样锁永远不会释放，导致死锁
1. 看你简历里说比较了解集合，对集合的了解简单说一下
集合（Set）是一种常见的数据结构。
...
1. 了解HashMap吗？长度为什么是2的幂？
1. HashMap的扩容机制
1. 为什么加载因子是0.75
1. HashMap什么时候把链表转换为红黑树？什么时候红黑树变为链表？
1. concurrentHashMap了解吗？实际项目中用过吗？
1. 聊一聊Java的垃圾收集机制吧
1. 怎样判断内存是否需要回收？（对象死亡的判断方法？）
1. 可达性分析是如何对对象进行标记的？三色标记法了解吗？
1. 垃圾回收的过程（针对某一个具体的垃圾收集器CMS或G1）
1. 清除的过程呢？
1. 标记清除算法的优点和缺点？
1. CMS标记阶段比较长，如果产生了过多的浮动垃圾，有可能会出现回收赶不上分配的情况，从而导致GC失败，这种要怎么解决？
1. JDK后续几个版本的垃圾收集器？更新了什么你了解吗？
1. CMS和G1的比对了解吗？为什么废除了CMS？
1. G1怎么做到指定具体的垃圾清除时间的？
1. 线程的生命周期你了解吗？
1. 每个状态之间怎么流转的你了解吗？
1. 线程池了解哪些？
1. 怎么确定线程池参数知道吗？
1. 手撕算法，相似题目力扣940不同的子序列Ⅱ
## 饿了么

#### 一面

1. 自我介绍
1. Java面向对象的三个特性，什么是多态
1. 双亲委派机制
1. HashMap在JDK1.7以前有线程不安全，怎么个情况
1. 保证线程安全用什么Map
1. ConcurrentHashMap是怎样的
1. 线程池用过吗？核心参数以及有啥作用
1. 使用线程池有什么好处
1. 悲观锁和乐观锁是怎么样子的
1. Java中怎么实现悲观锁和乐观锁
1. HTTP和HTTPS的区别，加密过程是怎样的
...
1. 接收方的应用程序读取数据的速度较慢，而发送方发送数据的速度较快，这会导致接收方缓冲区中积累多个数据包，当接收方读取时，可能会一次性读出多个数据包，形成粘包。
为了解决 TCP 粘包问题，可以在应用层采取一些措施，比如：

1. 明确数据包的边界：可以在数据包中添加特定的分隔符，接收方根据分隔符来区分不同的数据包。
1. 固定数据包的长度：发送方和接收方约定每个数据包的固定长度，接收方按照固定长度来读取数据包。
1. 在数据包头部添加长度字段：发送方在数据包头部指明数据包的长度，接收方根据长度来读取完整的数据包。
...
1. 两个项目哪个有挑战一点，挑一个讲一讲
1. 平时做项目或科研会遇到什么困难，遇到困难一般自己解决吗
1. 最近有了解一些新的技术或者看一些技术书籍之类的吗
1. 算法题：反转链表
1. 其他的offer
1. 之后有往上海发展的打算吗
1. 反问
