...
func (s *InterviewServiceImpl) SaveInterviewDialogues(ctx context.Context, userID uint, recordID uint64, questions []interface{}, dialogues []interface{}) error {
	...
	return model.WithTransaction(ctx, func(tx *gorm.DB) error {
		...
		for _, dialogue := range dialogueMap {
			...
			if dialogue.Question != "" || dialogue.Answer != "" {
				if err := tx.Create(dialogue).Error; err != nil {
					log.Printf("[SaveInterviewDialogues] 保存对话失败: %v", err)
					return err
				}
				savedCount++
			}
		...
		log.Printf("[SaveInterviewDialogues] 保存完成，共创建 %d 个维度主题，保存 %d 条对话记录", len(dimensionTopicMap), savedCount)
		return nil
	})
}
